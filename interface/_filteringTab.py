...
def showFiltering(callbacks):
    with dpg.group(horizontal=True):
        with dpg.child_window(width=300):
            ...
            dpg.add_slider_int(default_value=0, min_value=-100, max_value=100, tag='brightnessSlider', callback=lambda: callbacks.imageProcessing.executeQuery('brightnessAndContrast'))
            dpg.add_text('Contrast')
            dpg.add_slider_float(default_value=1.0, min_value=0.0, max_value=3.0, tag='contrastSlider', callback=lambda: callbacks.imageProcessing.executeQuery('brightnessAndContrast'))
            dpg.add_separator()
            ...
            dpg.add_slider_int(tag='averageBlurSlider', default_value=1, min_value=1, max_value=100, callback=lambda: callbacks.imageProcessing.executeQuery('averageBlur'))
            dpg.add_separator()
            ...
            dpg.add_text('Intensity')
            dpg.add_slider_int(tag='gaussianBlurSlider', default_value=1, min_value=1, max_value=100, callback=lambda: callbacks.imageProcessing.executeQuery('gaussianBlur'))
            dpg.add_separator()
            ...
            dpg.add_text('Intensity')
            dpg.add_slider_int(tag='medianBlurSlider', default_value=1, min_value=1, max_value=100, callback=lambda: callbacks.imageProcessing.executeQuery('medianBlur'))
                
            with dpg.group(tag="exportImageAsFileFilteringGroup", show=False):
                dpg.add_separator()
                dpg.add_text("Save Image")
                dpg.add_button(tag='exportImageAsFileFiltering', label='Export Image as File', callback=lambda sender, app_data: callbacks.imageProcessing.exportImage(sender, app_data, 'Filtering'))
